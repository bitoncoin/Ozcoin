...
if($show == "searchUsers"){
				...
				<?php
						...
						//Query for a list of users that match this username
							$searchQ = mysql_query("SELECT `accountLocked`, `email`, `username`, `id`, `loggedIp`, hashrate, share_count, stale_share_count FROM `webUsers` WHERE `username` LIKE '%".$searchUsername."%' OR `email` LIKE '%".$searchUsername."%'");
				?>
					...
					<input type="submit" value="Execute Changes"><br/>
						<?php
							...
							print("<tr>");
							print("<td align=left><a href=/userdetails.php?id=$user[id]>$user[id]</a></td>");
							print("<td align=left><a href=/userdetails.php?id=$user[id]>$user[username]</a></td>");
