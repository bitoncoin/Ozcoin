...
class Stats {
	...
	function lastwinningblocks($limit) {
		...
		if (!($uwa = getCache("last_winning_blocks"))) {
			$result = $read_only_db->query("SELECT username, blockNumber, confirms, timestamp FROM winning_shares ORDER BY blockNumber DESC LIMIT ".$limit);
			while ($row = $result->fetch()) {
				$uwa[$i] = Array();
				$uwa[$i][0] = $row[0];
				$uwa[$i][1] = $row[1];
	...
	function userhashrates() {
		...
		if (!($uwa = getCache("user_hashrates"))) {
			...
			while ($resultObj = $result->fetch()) {				
				//$uwa[$resultObj[1]] = round((($resultObj[0]*4294967296)/590)/1000000, 0);
				$uwa[$resultObj[1]] = round((($resultObj[0]*pow(2, 16))/600)/1000, 0);
			}
			if (count($uwa) > 0) 
