...
$validRegister = 0;
	$returnError .= "Password must include at least one CAPS! <br />";
			...
			if (strlen($username) > 20) {
				$validRegister = 0;
				$returnError .= " | Username is too long (20 max)";
			}
		...
		if($validRegister){
			...
			connectToDb();
			$result = mysql_query("INSERT INTO webUsers (`admin`, `username`, `pass`, `email`, `emailAuthPin`, `secret`, `loggedIp`, `sessionTimeoutStamp`, `accountLocked`, `accountFailedAttempts`, `pin`, api_key, joindate)
									VALUES ('0', '".$username."', '".hash("sha256",$pass.$salt)."', '".$email."', '".$emailAuthPin."', '".$secret."', '0', '0', '0', '0', '".hash("sha256", $authPin.$salt)."','".$apikey."','".get_date_time()."')");
...
<form action="/register.php" method="post">
	...
	<tr><td>Min password length is 6.</td></tr>
	<tr><td>Password:</td><td><input type="password" name="pass" value="" size="15" maxlength="200"></td></tr>
	<tr><td>Repeat Password:</td><td><input type="password" name="pass2" value="" size="15" maxlength="200"></td></tr>
